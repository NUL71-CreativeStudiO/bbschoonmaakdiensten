...
export const JobApplicationForm: React.FC = () => {
  ...
  const onSubmit = async (data: JobApplicationFormData) => {
    ...
    try {
      await sendJobApplication(data, "Schoonmaakmedewerker");
      setIsSubmitted(true);
      reset();
      setSelectedFile(null);
    } catch (error) {
              ...
              <AnimatePresence mode="wait">
                {isSubmitted ? (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                      <div>
                        ...
                        <input 
                          ...
                          className={`w-full px-4 py-3 rounded-lg bg-slate-50 border focus:ring-2 focus:ring-secondary focus:border-transparent outline-none transition-all font-medium ${errors.fullName ? 'border-red-500' : 'border-slate-200'}`}
                        />
                      ...
                      <div>
                        ...
                        <input 
                          ...
                          className={`w-full px-4 py-3 rounded-lg bg-slate-50 border focus:ring-2 focus:ring-secondary focus:border-transparent outline-none transition-all font-medium ${errors.city ? 'border-red-500' : 'border-slate-200'}`}
                        />
                    ...
                    <div>
                      <label className="block text-sm font-bold text-slate-700 mb-2">Motivatie</label>
                      <textarea 
                        rows={5}
                        {...register("motivation", { required: "Vertel ons kort waarom je deze baan wilt" })}
                        placeholder="Vertel ons waarom je graag bij ons wilt werken..."
                        className={`w-full px-4 py-3 rounded-lg bg-slate-50 border focus:ring-2 focus:ring-secondary focus:border-transparent outline-none transition-all font-medium ${errors.motivation ? 'border-red-500' : 'border-slate-200'}`}
                      ></textarea>
                    ...
                    {!selectedFile ? (
                      <div 
                        ...
                        className="border-2 border-dashed border-slate-200 rounded-lg p-6 text-center hover:bg-slate-50 hover:border-secondary/50 transition-all cursor-pointer group"
                      >
                        <div className="w-12 h-12 bg-slate-100 rounded-full flex items-center justify-center text-slate-400 mx-auto mb-3 group-hover:bg-secondary/10 group-hover:text-secondary transition-colors">
                          <UploadCloud size={24} />
                        </div>
                        <p className="text-sm text-slate-600 font-medium group-hover:text-secondary">Klik om CV te uploaden (Optioneel)</p>
                    ...
                    <div className="pt-4">
                      <Button type="submit" variant="secondary" fullWidth disabled={isSubmitting} className="text-lg py-4">
                        {isSubmitting ? 'Versturen...' : 'Sollicitatie Versturen'}
                      </Button>
                    </div>
